...
use reth_tasks::{TaskManager, TaskSpawner, TokioTaskExecutor};
...
impl RethLibmdbxClientBuilder {
    ...
    pub async fn build(self) -> eyre::Result<RethLibmdbxClient> {
        ...
        let task_manager = TaskManager::current().executor();

        crate::reth_libmdbx::init::new_with_db(db, self.max_tasks, task_manager, static_files).await
    }

    // pub async fn build_with_task_executor<T: TaskSpawner + Clone + 'static>(
    //     self,
    //     task_executor: T
    ...
    //     crate::reth_libmdbx::init::new_with_db(db, self.max_tasks, task_executor,
    // static_files).await }
}
